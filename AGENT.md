...
- Host CLIs are spawned without a shell on every platform (cross-spawn on Windows, so batch shims and shebang scripts work and arguments are escaped). Windows has no process groups: killing a host CLI ends its process tree with `taskkill /T /F`.
