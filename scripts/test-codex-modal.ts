  ...
  return new Sandbox("modal", {
    ...
    provider: {
      ...
      ...(process.env.MODAL_TOKEN_SECRET
        ...
        : {}),
      unencryptedPorts: [MODAL_CODEX_PORT],
    },
  });
}

function createAgent(sandbox: Sandbox<"modal">): Agent<"codex"> {
...
async function executeRun(
  ...
  try {
    ...
    void (async () => {
      try {
        for await (const raw of run.rawEvents()) {
          const method = raw.type ?? "unknown";
          const payload = raw.payload as Record<string, unknown> | undefined;
          const params = payload?.params as Record<string, unknown> | undefined;
          rawMethods.push(method);
          ...
          if (
            ...
            console.log(
              `  [${label}:raw] ${method} threadId=${threadId ?? "none"}`,
            );
          }
        }
      } catch {
        // raw stream may error on abort
      }
    })();
    ...
    console.log(
      `  [${label}:raw] all methods seen: ${[...new Set(rawMethods)].join(", ")}`,
    );
    return {
      label,
      success: false,
...
async function main() {
  console.log("=== Codex + Modal Sandbox Test ===\n");

  // --- Test 1: Single dummy run ---
  console.log("--- Test 1: Single Run ---");
  const sandbox1 = createSandbox("single-run");
  let singleResult: RunResult | undefined;

  try {
    const version = await sandbox1.run("codex --version", {
      cwd: "/workspace",
      timeoutMs: 30_000,
    });
    console.log(
      `  Codex version: ${version.stdout.trim() || version.combinedOutput.trim()}`,
    );
  ...
  try {
    const version = await sandbox2.run("codex --version", {
      cwd: "/workspace",
      timeoutMs: 30_000,
    });
    console.log(
